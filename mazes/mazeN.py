"""Grid-size-parameterised maze generator (maze.py / maze9.py with G as an argument).

Same construction: uniform wall density, a random open goal, exact BFS distances,
and the optimal-action mask. Kept separate so the published G=7 and G=9 runs are
untouched.
"""
import numpy as np
INF=10**6

def gen(M, G, density=0.30, seed=0):
