...
from gen import make, held_pairs, encode, TOK, V, CONSTS, FUNCS

CTX=140; DM=96; NL=3; NH=4; BS=64
...
def batch(bs, rng, split):
    ...
    for i in range(bs):
        t,ans,*_=make(DEPTH,rng,HELD,split)
        ids=encode(t)
...
if __name__=="__main__":
    ...
    print(f"depth {DEPTH}  |  {npar/1e6:.2f}M params  {NL} layers  {DM} dim  {NH} heads  ctx {CTX}")
    o=torch.optim.AdamW(net.parameters(),lr=3e-3,weight_decay=0.01)
    sch=torch.optim.lr_scheduler.OneCycleLR(o,3e-3,total_steps=STEPS,pct_start=0.1)
    ...
    torch.save({'n':net.state_dict()},f"ckpt/compose_d{DEPTH}.pt")
    R=json.load(open("../results/compose.json")) if os.path.exists("../results/compose.json") else {}
    R[str(DEPTH)]={"steps":STEPS,"params":npar,"seen":tr,"held":te,"hist":hist}
