...
class LM(nn.Module):
    def __init__(s, ncls, dm=96, nl=3):
        super().__init__()
...
def baseline(L):
    cl=code[tr_ids]>>(DEPTH-L); pr=np.bincount(cl,minlength=2**L)+1.0; pr/=pr.sum()
    vc=code[va_ids]>>(DEPTH-L)
...
def run(L, steps=None, bs=None, lr=3e-3, BUD=180, t0=0):
    ...
    for s in range(s0+1,steps+1):
        ...
        if time.time()-t0>BUD or s==steps:
            ...
            if s<steps: print(f"  L{L} paused {s}/{steps} loss {loss.item():.3f}",flush=True); return None
