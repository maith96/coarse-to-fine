...
from gatelm import LM, batch, tgt, tr_ids, va_ids, baseline
from chain import expand, ev

L=13; NC=2**L; BS=16
def run(cond, steps, seed, BUD, t0):
    key=f"{cond}_n{steps}_s{seed}"; ck=f"ckpt/sw_{key}.pt"
    torch.manual_seed(seed); net=LM(NC)
    if cond=="anc" and not os.path.exists(ck):
        net.load_state_dict(expand(torch.load("ckpt/ch_anc_L12.pt")['n'],12,13,NC))
...
if __name__=="__main__":
    ...
    for cond,n,sd in jobs:
        ...
        res[k]=r; json.dump(res,open("sweep.json","w"))
