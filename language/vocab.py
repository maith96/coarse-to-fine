import re, numpy as np, collections, json
from scipy.sparse import coo_matrix
from scipy.sparse.linalg import svds

txt=open("shake.txt").read().lower()
...
# balanced recursive 2-means -> 13-bit hierarchical code (prefix property exact)
DEPTH=13
...
cl4=code>>(DEPTH-4)
for b in [0,7]:
    ws=[("<unk>"+words)[0] if i==0 else words[i-1] for i in np.where(cl4==b)[0][:14]]
    print(f"  cluster{b}: "+" ".join(ws))
